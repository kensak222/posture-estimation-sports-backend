...
class PostureEstimationService:
    def __init__(self):
        model_path = "ai_model/move_net_thunder_fp16/"
        self.model = tf.saved_model.load(model_path)
        # 推論関数を取得
        self.infer = self.model.signatures["serving_default"]

    def estimate_pose(self, image_path: str, output_path: str):
        """
        指定された画像に姿勢推定を適用する。
        """
        ...
        # 保存処理
        # OpenCVではRGB画像をBGRに変換する必要がある
        output_overlay_bgr = cv2.cvtColor(output_overlay, cv2.COLOR_RGB2BGR)
        success = cv2.imwrite(output_path, output_overlay_bgr)

        if success:
            logger.info(f"姿勢推定画像生成完了: {output_path}")
        else:
    ...
    def draw_prediction_on_image(
        ...
        if len(image.shape) == 2:  # グレースケール画像
            ...
            channel = 1
            image = cv2.cvtColor(image, cv2.COLOR_GRAY2RGB)  # RGB変換
        ...
        (keypoint_locs, keypoint_edges, edge_colors) = (
            self.keypoints_and_edges_for_display(keypoints_with_scores, height, width)
        )
    ...
    def keypoints_and_edges_for_display(
        self, keypoints_with_scores, height, width, keypoint_threshold=0.11
    ):
